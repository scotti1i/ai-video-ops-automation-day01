...
**零密钥跑的是演示**：样例数据、模拟平台，30 秒看清产品长什么样。**真实使用要接真东西**：你自己的模型（OpenAI / 本机 claude / codex 命令行）和 YouTube 账号——这条真实路径已经真跑通并留了证据，不是录屏里的假接口。
...
需要 Python 3.12+、Node.js、npm。共享界面已打包进 `vendor/`，不依赖任何仓库外的目录。这一步跑的是**演示模式**——样例数据 + 模拟平台，用来看清整条流水线；真实干活请接着看「接真实能力」。
...
演示模式永远不需要密钥，但它产出的是样例，不是你的业务。**要真的用起来**，配好环境变量后用 `uv run video-ops run`——下面每一条都是验证过的真实路径。字段模板见 [`config.example.env`](config.example.env)；界面的「连接与配置」页会如实显示每个引擎 / 平台当前能不能用、缺什么、怎么配。
